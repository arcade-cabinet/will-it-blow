...
describe('Full game scoring pipeline', () => {
  ...
  it('worst-case ingredients still produce valid output', () => {
    ...
    const worst = INGREDIENTS.filter(i => i.category === 'absurd' && i.tasteMod <= 0);
  ...
  it('best-case ingredients with bonus can reach THE SAUSAGE KING', () => {
    ...
    const best = INGREDIENTS.filter(i => i.tasteMod >= 4 && i.textureMod >= 3);
...
describe('Game balance sanity checks', () => {
  it('high-burst ingredients have lower taste (risk/reward trade-off)', () => {
    ...
    const avgHighTaste = highBurst.reduce((a, i) => a + i.tasteMod, 0) / highBurst.length;
    const avgLowTaste = lowBurst.reduce((a, i) => a + i.tasteMod, 0) / lowBurst.length;
  ...
  it('ingredient pool has a balanced tier distribution', () => {
    ...
    const decent = INGREDIENTS.filter(i => i.tasteMod >= 2 && i.tasteMod < 4).length;
