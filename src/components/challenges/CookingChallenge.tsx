...
export function CookingChallenge({ onComplete, onReaction }: CookingChallengeProps) {
	...
	const [overheatCount, setOverheatCount] = useState(0);
		...
		const interval = setInterval(() => {
			...
			if (newTime <= 0) {
				...
				const score = Math.max(
					...
					Math.round(
						holdPct * 100 -
						overheatCountRef.current * SCORE_PENALTY_PER_OVERHEAT,
					),
				);
				onComplete(score);
			}
	...
	}, [phase, variant, addStrike, setChallengeProgress, setChallengeTemperature, setChallengeHeatLevel, onComplete, onReaction]);
				...
				<>
					...
					<View style={styles.tempContainer}>
						...
						<Text style={styles.targetLabel}>
							TARGET: {variant.targetTemp}{'\u00B0'}F {'\u00B1'} {variant.tempTolerance}{'\u00B0'}F
						</Text>
						{inTargetZone && (
					...
					>
						...
						<View style={styles.sliderTrack}>
							...
							<View style={[
								...
								{
									height: `${heatFillPct}%`,
									backgroundColor: heatLevel > 0.7 ? '#FF1744' : heatLevel > 0.4 ? '#FFC107' : '#FF9800',
								},
							]} />
							{/* Slider thumb */}
							<View style={[
					...
					{overheatFlash && (
						<View style={styles.overheatOverlay} pointerEvents="none" />
					)}

					{/* Heat level indicator */}
					<View style={styles.heatIndicator}>
						<Text style={styles.heatIndicatorText}>
							HEAT: {Math.round(heatLevel * 100)}%
						</Text>
					</View>
				</>
			)}
