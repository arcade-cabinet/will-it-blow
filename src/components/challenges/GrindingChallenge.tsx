...
export function GrindingChallenge({ onComplete, onReaction }: GrindingChallengeProps) {
	...
	const calculateAngle = useCallback((pageX: number, pageY: number, layout: { cx: number; cy: number }) => {
			...
			onPanResponderMove: (evt) => {
				...
				if (prevAngleRef.current !== null) {
					...
					if (deltaTime > 0.001) {
						...
						const smoothed = EMA_ALPHA * instantVelocity + (1 - EMA_ALPHA) * smoothedVelocityRef.current;
	...
	const handleSuccessComplete = useCallback(() => {
		...
		const score = Math.max(
			0,
			Math.round(100 - strikesRef.current * SCORE_PENALTY_PER_STRIKE - overtime * SCORE_PENALTY_PER_OVERTIME_SEC),
		);
	...
	const goodZoneStart = variant
		? ((variant.targetSpeed - variant.tolerance) / (variant.targetSpeed + variant.tolerance + 1)) * 100
		: 30;
		...
		<View style={styles.container} pointerEvents="box-none">
			...
			{phase === 'grinding' && (
				<>
					...
					{isSplattering && (
						<View style={styles.splatterOverlay} pointerEvents="none" />
					)}
