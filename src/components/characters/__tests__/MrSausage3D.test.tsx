...
describe('MrSausage3D', () => {
  it('renders without crashing', async () => {
    const renderer = await ReactThreeTestRenderer.create(<MrSausage3D reaction="idle" />);
  ...
  it('accepts all valid reactions without error', async () => {
    ...
    for (const reaction of reactions) {
      const renderer = await ReactThreeTestRenderer.create(<MrSausage3D reaction={reaction} />);
      expect(renderer.scene.children.length).toBeGreaterThan(0);
