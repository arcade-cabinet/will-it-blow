...
export function ScatterProps() {
  ...
  const scatterList = useMemo(() => [
    ...
    { node: nodes.Pizza_Slice, mat: materials.Pizza_Slice_Material, pos: [-1.2, 3, -0.8], rot: [0, 0, 0] },
    { node: nodes.Radio, mat: materials.Radio_Material, pos: [1.5, 3, -2.0], rot: [0, 0.2, 0] },
    { node: nodes.Poster_Missing, mat: materials.Poster_Missing_Material, pos: [0, 3, 2.0], rot: [0, 0, 0] },
    ...
    { node: nodes.Cigarette_Pack, mat: materials.Cigarette_Pack_Material, pos: [-0.5, 3, 1.5], rot: [0, 0, 0] },
    { node: nodes.Battery, mat: materials.Battery_Material, pos: [1.2, 3, -0.5], rot: [0, 0, 0] },
    { node: nodes.Bottle_1, mat: materials.Bottle_1_Material, pos: [-2.2, 3, 1.5], rot: [Math.PI/2, 0, 0] },
  ], [nodes, materials]);

  return (
    <group>
      {scatterList.map((item, i) => item.node && (
        <RigidBody key={i} position={item.pos as any} rotation={item.rot as any} colliders="hull" mass={0.1}>
          <mesh geometry={item.node.geometry} material={item.mat} castShadow receiveShadow scale={1.5} />
        </RigidBody>
      ))}
    </group>
  );
