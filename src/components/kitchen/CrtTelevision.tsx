...
export const CrtTelevision = ({
	...
	useFrame((_, delta) => {
		...
		const flicker =
			1.0 + Math.sin(s.time * 4) * 0.3 * (1.0 + s.currentReactionIntensity);
	...
	return (
		<group position={position}>
			...
			<mesh position={[0, 0, TV.bezelZ - 0.04]}>
				<boxGeometry
					args={[TV.bezel.width + 0.15, TV.bezel.height + 0.15, 0.08]}
				/>
			...
			<mesh position={[0, fh, TV.bezelZ]}>
				<boxGeometry
					args={[TV.bezel.width + frameThickness * 2, frameThickness, TV.bezel.depth]}
				/>
			...
			<mesh position={[-fw, 0, TV.bezelZ]}>
				<boxGeometry
					args={[frameThickness, TV.bezel.height, TV.bezel.depth]}
				/>
				<meshStandardMaterial color={[0.08, 0.07, 0.06]} roughness={0.9} />
			</mesh>
			{/* Right */}
			...
			<mesh
				position={[TV.housing.width / 2 - 0.2, 0.3, TV.bezelZ]}
