  ...
  return (
    <group position={FRIDGE_POS}>
      ...
      <mesh position={[0, 0, 0]}>
        ...
        <meshStandardMaterial color={[0.18, 0.17, 0.15]} roughness={0.9} metalness={0.05} />
      </mesh>

      {/* ===== Open door (pivoted ~90 degrees to the left) ===== */}
      <mesh position={[-0.75, 0, 0.75]} rotation={[0, Math.PI / 2, 0]}>
        <boxGeometry args={[1.5, 3, 0.08]} />
        <meshStandardMaterial color={[0.16, 0.15, 0.14]} roughness={0.9} metalness={0.05} />
      </mesh>

      {/* ===== Interior light glow (emissive plane at top of fridge) ===== */}
      <mesh position={[0, 1.4, 0.49]}>
        <planeGeometry args={[1.3, 0.15]} />
        <meshBasicMaterial color={[0.3, 0.35, 0.5]} transparent opacity={0.5} />
      </mesh>

      {/* ===== Interior point light ===== */}
      <pointLight position={[0, 1.0, 0.2]} intensity={0.5} color="#e0f0ff" distance={3} decay={2} />

      {/* ===== Shelves ===== */}
      {SHELF_Y_POSITIONS.map((shelfY, s) => (
        <mesh key={`shelf_${s}`} position={[0, -1.5 + shelfY, 0]}>
          <boxGeometry args={[1.4, 0.05, 0.85]} />
          <meshStandardMaterial color={[0.2, 0.22, 0.25]} roughness={0.9} metalness={0.05} />
        </mesh>
      ))}
      ...
      {ingredients.map((ingredient, i) => {
        ...
        const shelfY = -1.5 + SHELF_Y_POSITIONS[shelfIndex] + INGREDIENT_DIAMETER / 2 + 0.05;
