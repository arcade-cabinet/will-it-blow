...
const BURST_PARTICLE_SIZES = Array.from({ length: BURST_PARTICLE_COUNT }, (_, i) => (
	0.03 + (i * 0.005) % 0.04
));
...
export const StufferStation = ({
	...
	const burstStateRef = useRef({
		active: false,
		time: 0,
		velocities: Array.from({ length: BURST_PARTICLE_COUNT }, () => [0, 0, 0] as [number, number, number]),
		positions: Array.from({ length: BURST_PARTICLE_COUNT }, () => [0, 0, 0] as [number, number, number]),
	});
	...
	return (
		<group position={STUFFER_POS}>
			...
			<mesh
				ref={handleRef}
				position={[0, STUFFER_BASE_Y + BODY_HEIGHT + HANDLE_HEIGHT / 2, 0]}
			>
			...
			<mesh
				ref={handleKnobRef}
				position={[0, STUFFER_BASE_Y + BODY_HEIGHT + HANDLE_HEIGHT, 0]}
			>
			...
			<mesh
				ref={casingRef}
				position={[BODY_DIAMETER / 2 + 0.3 + CASING_BASE_LENGTH / 2, STUFFER_BASE_Y + BODY_HEIGHT * 0.25, 0]}
				rotation={[0, 0, Math.PI / 2]}
			>
				<cylinderGeometry args={[CASING_BASE_DIAMETER / 2, CASING_BASE_DIAMETER / 2, CASING_BASE_LENGTH, 16]} />
				<meshBasicMaterial
					ref={casingMatRef}
					color={[0.2, 0.7, 0.15]}
					transparent
			...
			<mesh
				ref={casingEndRef}
				position={[BODY_DIAMETER / 2 + 0.3 + CASING_BASE_LENGTH, STUFFER_BASE_Y + BODY_HEIGHT * 0.25, 0]}
			>
			...
			<mesh
				ref={meatFillRef}
				position={[0, STUFFER_BASE_Y + BODY_HEIGHT * 0.45, 0]}
			>
				<cylinderGeometry args={[BODY_DIAMETER * 0.425, BODY_DIAMETER * 0.425, BODY_HEIGHT * 0.9, 16]} />
