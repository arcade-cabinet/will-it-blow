...
describe('pan removal records cook level', () => {
  ...
  it('store recordCookLevel action writes to cookLevel state', () => {
    const store = useGameStore.getState();
    expect(store.cookLevel).toBe(0);
    store.recordCookLevel(0.65);
    expect(useGameStore.getState().cookLevel).toBe(0.65);
  });
});
...
describe('flip records flair points', () => {
  ...
  it('store recordFlairPoint action accumulates points', () => {
    const store = useGameStore.getState();
    expect(store.flairPoints).toBe(0);
    store.recordFlairPoint('pan-flip', 3);
    expect(useGameStore.getState().flairPoints).toBe(3);
    store.recordFlairPoint('pan-flip', 2);
    expect(useGameStore.getState().flairPoints).toBe(5);
  });

  it('startNewGame resets cookLevel and flairPoints', () => {
    const store = useGameStore.getState();
    store.recordCookLevel(0.7);
    store.recordFlairPoint('pan-flip', 3);
    expect(useGameStore.getState().cookLevel).toBe(0.7);
    expect(useGameStore.getState().flairPoints).toBe(3);
    store.startNewGame();
    expect(useGameStore.getState().cookLevel).toBe(0);
    expect(useGameStore.getState().flairPoints).toBe(0);
  });
});
