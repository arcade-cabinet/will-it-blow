...
export function Stove() {
  ...
  const normMat = useMemo(() => new THREE.ShaderMaterial({ 
    uniforms: { 
      u_height: {value:null}, 
      u_texelSize: {value:new THREE.Vector2(1/fboSize, 1/fboSize)}, 
      u_scale: {value:2.5} 
  ...
  useEffect(() => {
    ...
    return () => { fboScene.remove(fboQuad); };
  }, [fboScene, fboQuad]);
  ...
  const splatMesh = useMemo(() => {
    ...
    if(sCtx) {
      sCtx.canvas.width=32; sCtx.canvas.height=32; 
  ...
  useEffect(() => {
    ...
    return () => { splatScene.remove(splatMesh); };
  }, [splatScene, splatMesh]);

  const splatDummy = useMemo(() => new THREE.Object3D(), []);
  
  const greasePoolMat = useMemo(() => new THREE.MeshPhysicalMaterial({ 
    color: 0xcca600, 
    transparent: true, 
    opacity: 0.8, 
    roughness: 0.05, 
  ...
  const bindPan = useDrag(({ active, movement: [x, y] }) => {
    ...
    if (active) {
      ...
      if (new THREE.Vector3(0.8 + x * 0.01, 0, y * 0.01).distanceTo(new THREE.Vector3(0, 0, 0.8)) < 0.5) {
  ...
  useFrame((state, delta) => {
    if (gamePhase === 'COOKING') {
      ...
      greasePoolMat.color.lerpColors(new THREE.Color(0xcca600), new THREE.Color(0x8a5a00), cookLevel);
    }
  });
      ...
      <group position={[-0.4, 0.9, -0.4]}>
        ...
        {pan.scene && (
          ...
          <group {...bindPan()} position={panPos}>
            ...
            {(gamePhase === 'COOKING' || gamePhase === 'DONE') && burnerLevels[0] > 0 && panPos.z > 0.5 && (
