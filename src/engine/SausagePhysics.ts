...
export function calculateBlowRuffalos(
	...
	const avgBlow =
		ingredients.reduce((a, i) => a + i.blowPower, 0) / ingredients.length;
...
export function checkBurst(ingredients: Ingredient[]): boolean {
	const avgRisk =
		ingredients.reduce((a, i) => a + i.burstRisk, 0) / ingredients.length;
...
export function calculateTasteRating(
	...
	const avgTaste =
		ingredients.reduce((a, i) => a + i.tasteMod, 0) / ingredients.length;
	const avgTexture =
		ingredients.reduce((a, i) => a + i.textureMod, 0) / ingredients.length;
...
export function calculateFinalScore(
	...
	return Math.min(
		Math.round(tasteScore + blowScore + burstBonus + bonusPoints),
		100,
	);
}
